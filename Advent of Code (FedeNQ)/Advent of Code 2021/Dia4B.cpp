...
int main()
{
	...
	int winner = -1, val=-1, winners = MAXN;
	forn(act,tam){
		forn(pos, MAXN){
			if(won[pos]) continue;
			int x=-1,y=-1;
			...
			if(row[pos][x] == 5 || col[pos][y] == 5){
				cout << "GANO " << pos << '\n';
				won[pos]=1; winners--;
				if(winners==0){ won[pos]=0; val=orden[act];}
			}
			if(winners==0) break;
		}
		if(winners==0) break;
	...
	int acum = 0;
	forn(i,MAXN) if(won[i]==0) winner=i;
	cout << winner << " " << val << '\n';
	forn(i,5){
		forn(j,5){
